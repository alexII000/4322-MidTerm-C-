...
class DinnerPlanner:
    def __init__(self):
        ...
        self.numberLabel = tkinter.Label(self.frame5,text="Number in your party:")
        ...
        self.radio_var.set('3/1/23')

        self.dateRB1 = tkinter.Radiobutton(self.frame8,text='March 1 2023',variable=self.radio_var,value='3/1/23')
        self.dateRB2 = tkinter.Radiobutton(self.frame8,text='March 2 2023',variable=self.radio_var,value='3/2/23')
        self.dateRB3 = tkinter.Radiobutton(self.frame8,text='March 3 2023',variable=self.radio_var,value='3/3/23')
        ...
        self.optionsLabel = tkinter.Label(self.frame10,text='Please select all that apply:')
        self.option1 = tkinter.Checkbutton(self.frame11,text='Gluten Free   ',variable=self.cb_var1)
        self.option2 = tkinter.Checkbutton(self.frame11,text='Vegan         ',variable=self.cb_var2)
        self.option3 = tkinter.Checkbutton(self.frame11,text='Peanut Allergy',variable=self.cb_var3)
        ...
        self.submitButton = tkinter.Button(self.frame13,text="Submit",command=self.reserve)
        self.dummyLabel6 = tkinter.Label(self.frame13,text="",width=20)
        self.cancelButton = tkinter.Button(self.frame13,text="Cancel",command=self.main_window.destroy)
    ...
    def reserve(self):
        ...
        if not self.nameEntry.get():
            tkinter.messagebox.showinfo('Missing Info','Please fill out your name')
            reserve = False

        if not self.phoneEntry.get():
            tkinter.messagebox.showinfo('Missing Info','Please give us your phone number')
            reserve = False
        
        if not self.numberEntry.get():
            tkinter.messagebox.showinfo('Missing Info','Please tell us how many in your party')
            reserve = False
        
        if int(self.numberEntry.get()) > 7:
            tkinter.messagebox.showinfo('Cannot accomodate','Sorry we cannot accomodate parties larger than 7')
            reserve = False

        if self.radio_var.get() == '3/1/23':
            tkinter.messagebox.showinfo('SOLD OUT','Sorry we are sold out for ' + self.radio_var.get())
            reserve = False

        if self.cb_var1.get() == 1:
            tkinter.messagebox.showinfo('Info','There is an additional charge of $2.00 for gluten-free options')
